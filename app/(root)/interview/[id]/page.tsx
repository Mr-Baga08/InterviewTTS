...
const InterviewDetails = async ({ params }: RouteParams) => {
  ...
  // Fetch interview data with proper ActionResult handling
  const interviewResult = await getInterviewById(id);
  
  // Handle ActionResult for interview
  if (!interviewResult.success || !interviewResult.data) {
    console.error("Interview not found or error:", interviewResult.message);
    redirect("/");
  }

  const interview = interviewResult.data;

  // Fetch feedback data with proper ActionResult handling
  const feedbackResult = await getFeedbackByInterviewId({
    interviewId: id,
    userId: user.id,
  });

  // Handle ActionResult for feedback (feedback might not exist, which is okay)
  ...
  const getInterviewStatus = (): InterviewStatusInfo => {
    ...
    // Check if interview was recently created (within last hour) as an indicator
  ...
  const questionsAsStrings = React.useMemo(() => {
    ...
    return interview.questions.map((question: any) => {
  ...
  const handleViewFeedback = () => {
    // Using Next.js router would be better, but for now using window.location
  ...
  const getStatusIcon = (iconType: string) => {
    const icons = {
      ...
      "sparkles": (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3a2 2 0 00-2 2v1c0 8.284 6.716 15 15 15h1a2 2 0 002-2V3H5z" />
                ...
                <div className="relative">
                  <div className="w-16 h-16 rounded-2xl overflow-hidden border-2 border-white/10 bg-gradient-to-br from-white/10 to-white/5">
                    <Image
                      ...
                      alt={`${interview.role} interview cover`}
                      width={64}
                      height={64}
